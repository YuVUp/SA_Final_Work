...
NFUSA2 Пользователь находит и понимает информацию о блюде менее, чем за 3 секунды с вероятностью 95% без прокрутки и в стандартном формате.
...
- Инциденте зафиксированы в системе
...
6.а. Посетитель выбирает опцию "Добавить несколько"
    ...
    5. Возврат к шагу 8 основного потока
...
7.а. Требуемый ингредиент отсутствует в справочнике
    ...
    3. Менеджер заполняет параметры ингредиента
...
5. Use case завершается, когда посетитель либо переходит к оформлению заказа (нажимает «Сделать заказ»), либо выходит из меню
...
#### 6.2.5 Пользовательский путь (DOR, DOD)
